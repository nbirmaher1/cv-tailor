...
The **Job Search** page finds real, currently-open roles you're a match for. Give it a location (required) and optionally target titles, companies, or extra sites; it reads your master CV, casts a wide net across company ATS boards (Greenhouse, Lever, Ashby, Workday, and more) plus the usual aggregators for discovery, then verifies each candidate against its authoritative source before showing it — a two-step check (an authoritative fetch, then an independent review pass) so every result is a real, live posting, never a fabricated or stale one. Runs in the background like a tailoring run. Each result is scored for CV fit (0–100, shown as a colored `strong`/`possible`/`stretch` badge) and the list is sorted best-fit first; partial matches you have a plausible interview shot at are included, not only perfect ones. Each card explains why it fits and reads the role's key requirements against your CV, with a one-click **Tailor my CV for this** that hands the posting straight to the tailoring flow.
